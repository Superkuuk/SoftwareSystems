...
public class Card
{
	...
	public static Card read(BufferedReader in) throws EOFException {
		try (Scanner scanner = new Scanner(in.readLine())) {
			...
			char suit = '\u0000';
			if (scanner.hasNext()) {
				suit = suitString2Char(scanner.next());
			}
			if (scanner.hasNext()) {
				rank = rankString2Char(scanner.next());
			}			
			scanner.close();
	...
	public void write(DataOutput w) throws IOException {
		w.writeChar(this.getSuit());
		w.writeChar(this.getRank());
		((DataOutputStream) w).flush();
	}
	...
	public void write(ObjectOutput w) throws IOException {
		w.writeObject(this);
		w.flush();
	}
    ...
    */
    ...
    public static void main(String[] args) throws FileNotFoundException {
    	...
    	if (args.length == 0) {
    		// no arguments given, use System.out instead.
    	...
    	Card c1 = new Card(SPADES, rankString2Char("8"));
    	Card c2 = new Card(CLUBS, rankString2Char("2"));
    	Card c3 = new Card(HEARTS, JACK);
    	Card c4 = new Card(DIAMONDS, QUEEN);
